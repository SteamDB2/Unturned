...
public class RadiusModifier : MonoModifier
{
	...
	{
		Vector3 vector = new Vector3((float)Math.Cos((double)start), 0f, (float)Math.Sin((double)start)) * rad + p1;
		for (float num = start; num < end; num += 0.03141593f)
		{
			Vector3 vector2 = new Vector3((float)Math.Cos((double)num), 0f, (float)Math.Sin((double)num)) * rad + p1;
			Debug.DrawLine(vector, vector2, col);
			vector = vector2;
		}
		if ((double)end == 6.2831853071795862)
		{
			Vector3 vector3 = new Vector3((float)Math.Cos((double)start), 0f, (float)Math.Sin((double)start)) * rad + p1;
