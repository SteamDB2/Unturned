...
namespace Pathfinding.RVO.Sampled
{
	public class Agent : IAgent
	{
		...
		internal void CalculateVelocity(Simulator.WorkerContext context)
		{
			...
			Vector2 vector;
			vector..ctor(this.position.x, this.position.z);
			Agent.VO[] vos = context.vos;
			int num = 0;
			Vector2 vector2;
			vector2..ctor(this.velocity.x, this.velocity.z);
			...
			for (int j = 0; j < this.neighbours.Count; j++)
			{
				...
				if (agent != this)
				{
					...
					{
						Vector2 vector3;
						vector3..ctor(agent.Velocity.x, agent.velocity.z);
			...
			if (this.simulator.algorithm == Simulator.SamplingAlgorithm.GradientDecent)
			{
				if (this.DebugDraw)
				{
					for (int k = 0; k < 40; k++)
					{
						...
						{
							Vector2 vector7;
							vector7..ctor((float)k * 15f / 40f, (float)l * 15f / 40f);
							Vector2 vector8 = Vector2.zero;
							float num10 = 0f;
							for (int m = 0; m < num; m++)
							{
								float num11 = 0f;
								vector8 += vos[m].Sample(vector7 - vector, out num11);
								if (num11 > num10)
								{
									num10 = num11;
								}
							}
							Vector2 vector9 = new Vector2(this.desiredVelocity.x, this.desiredVelocity.z) - (vector7 - vector);
							vector8 += vector9 * Agent.DesiredVelocityScale;
							if (vector9.magnitude * Agent.DesiredVelocityWeight > num10)
							{
								num10 = vector9.magnitude * Agent.DesiredVelocityWeight;
							}
							if (num10 > 0f)
							{
								vector8 /= num10;
							}
							Debug.DrawRay(Agent.To3D(vector7), Agent.To3D(vector9 * 0f), Color.blue);
							float num12 = 0f;
							Vector2 vector10 = vector7 - Vector2.one * 15f * 0.5f;
							Vector2 vector11 = this.Trace(vos, num, vector10, 0.01f, out num12);
							if ((vector10 - vector11).sqrMagnitude < this.Sqr(0.375f) * 2.6f)
							{
								Debug.DrawRay(Agent.To3D(vector11 + vector), Vector3.up * 1f, Color.red);
							}
						}
					}
				}
				...
				Vector2 vector12;
				vector12..ctor(this.velocity.x, this.velocity.z);
				float cutoff = vector12.magnitude * this.simulator.qualityCutoff;
				...
				float num14;
				Vector2 vector13 = this.Trace(vos, num, p3, cutoff, out num14);
				if (num14 < num13)
				{
					vector6 = vector13;
					num13 = num14;
				...
				if (this.DebugDraw)
				{
					Agent.DrawCross(vector13 + vector, Color.magenta, 0.5f);
				}
			}
			else
			{
				...
				int num15 = 0;
				Vector2 vector14;
				vector14..ctor(this.desiredVelocity.x, this.desiredVelocity.z);
				float num16 = Mathf.Max(this.radius, Mathf.Max(vector14.magnitude, this.Velocity.magnitude));
				samplePos[num15] = vector14;
				sampleSize[num15] = num16 * 0.3f;
				num15++;
				samplePos[num15] = vector2;
				sampleSize[num15] = num16 * 0.3f;
				num15++;
				Vector2 vector15 = vector2 * 0.5f;
				Vector2 vector16;
				vector16..ctor(vector15.y, -vector15.x);
				for (int n = 0; n < 8; n++)
				{
					samplePos[num15] = vector16 * Mathf.Sin((float)n * 3.14159274f * 2f / 8f) + vector15 * (1f + Mathf.Cos((float)n * 3.14159274f * 2f / 8f));
				...
				{
					samplePos[num15] = vector16 * Mathf.Cos(((float)num17 + 0.5f) * 3.14159274f * 2f / 6f) + vector15 * (1.66666663f + Mathf.Sin(((float)num17 + 0.5f) * 3.14159274f * 2f / 6f));
					sampleSize[num15] = num16 * 0.3f;
					num15++;
				}
				...
				for (int num21 = 0; num21 < 3; num21++)
				{
					for (int num22 = 0; num22 < num15; num22++)
					{
						...
						float magnitude = (samplePos[num22] - vector14).magnitude;
					...
					{
						Vector2 vector18 = bestPos[num27];
						float num28 = bestSizes[num27];
						bestScores[num27] = float.PositiveInfinity;
						float num29 = num28 * 0.6f * 0.5f;
						samplePos[num15] = vector18 + new Vector2(num29, num29);
						samplePos[num15 + 1] = vector18 + new Vector2(-num29, num29);
						samplePos[num15 + 2] = vector18 + new Vector2(-num29, -num29);
						samplePos[num15 + 3] = vector18 + new Vector2(num29, -num29);
						num28 *= num28 * 0.6f;
		...
		private Vector2 Trace(Agent.VO[] vos, int voCount, Vector2 p, float cutoff, out float score)
		{
			...
			for (int i = 0; i < 50; i++)
			{
				...
				{
					float num4;
					Vector2 vector2 = vos[j].Sample(p, out num4);
					vector += vector2;
					if (num4 > num3)
					{
						num3 = num4;
				...
				Vector2 vector3 = new Vector2(this.desiredVelocity.x, this.desiredVelocity.z) - p;
				float val = vector3.magnitude * Agent.DesiredVelocityWeight;
				vector += vector3 * Agent.DesiredVelocityScale;
				...
				if (this.DebugDraw)
				{
					Debug.DrawLine(Agent.To3D(p2) + this.position, Agent.To3D(p) + this.position, Agent.Rainbow(0.1f / score) * new Color(1f, 1f, 1f, 0.2f));
				}
			}
			score = num;
			return result;
		...
		public struct VO
		{
			...
			public VO(Vector2 center, Vector2 offset, float radius, Vector2 sideChooser, float inverseDt, float weightFactor)
			{
				...
				if (center.magnitude < radius)
				{
					...
					Vector2 vector;
					vector..ctor(this.line1.y, -this.line1.x);
				...
				else
				{
					...
					Vector2 vector3;
					vector3..ctor(-this.cutoffLine.y, this.cutoffLine.x);
