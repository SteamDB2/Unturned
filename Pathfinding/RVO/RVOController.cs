...
namespace Pathfinding.RVO
{
	...
	public class RVOController : MonoBehaviour
	{
		...
		public void Update()
		{
			...
			if (this.mask != 0 && Physics.Raycast(interpolatedPosition + Vector3.up * this.height * 0.5f, Vector3.down, ref raycastHit, float.PositiveInfinity, this.mask))
			{
			...
			if (this.wallAvoidFalloff > 0f && this.wallAvoidForce > 0f)
			{
				...
				if (neighbourObstacles != null)
				{
					for (int i = 0; i < neighbourObstacles.Count; i++)
					{
						...
						Vector3 vector2 = this.position - AstarMath.NearestPointStrict(position, position2, this.position);
						if (!(vector2 == position) && !(vector2 == position2))
						{
							float sqrMagnitude = vector2.sqrMagnitude;
							vector2 /= sqrMagnitude * this.wallAvoidFalloff;
							vector += vector2;
						}
					}
				}
			}
			this.rvoAgent.DesiredVelocity = this.desiredVelocity + vector * this.wallAvoidForce;
