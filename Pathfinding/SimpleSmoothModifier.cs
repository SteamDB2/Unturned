	...
	public class SimpleSmoothModifier : MonoModifier
	{
		...
		public List<Vector3> CurvedNonuniform(List<Vector3> path)
		{
			...
			for (int j = 0; j < path.Count - 1; j++)
			{
				...
				Vector3 tan = vector2 * magnitude2 * this.factor;
				Vector3 tan2 = vector3 * magnitude2 * this.factor;
				Vector3 a = path[j];
				Vector3 b = path[j + 1];
				...
				{
					float t = num4 * num3;
					list.Add(SimpleSmoothModifier.GetPointOnCubic(a, b, tan, tan2, t));
				}
				vector = vector3;
			}
		...
		public List<Vector3> SmoothOffsetSimple(List<Vector3> path)
		{
			...
			for (int k = 0; k < this.iterations; k++)
			{
				...
				for (int l = 0; l < num2 - 1; l++)
				{
					...
					if (flag3)
					{
						list[l * 2] = vector + ((!flag) ? (-normalized * this.offset * num3) : (normalized * this.offset * num3));
					}
					else
					{
						list[l * 2] = vector;
					...
					if (flag4)
					{
						list[l * 2 + 1] = vector2 + ((!flag2) ? (-normalized * this.offset * num3) : (normalized * this.offset * num3));
					}
					else
					{
						list[l * 2 + 1] = vector2;
		...
		public List<Vector3> SmoothSimple(List<Vector3> path)
		{
				...
				if (this.strength != 0f)
				{
					for (int l = 0; l < this.iterations; l++)
					{
						...
						for (int m = 1; m < list.Count - 1; m++)
						{
							...
							list[m] = Vector3.Lerp(vector3, (vector2 + list[m + 1]) / 2f, this.strength);
							vector2 = vector3;
						}
					}
				}
			...
			for (int num10 = 0; num10 < this.iterations; num10++)
			{
				...
				for (int num11 = 1; num11 < list2.Count - 1; num11++)
				{
					...
					list2[num11] = Vector3.Lerp(vector5, (vector4 + list2[num11 + 1]) / 2f, this.strength);
					vector4 = vector5;
				}
			}
			return list2;
