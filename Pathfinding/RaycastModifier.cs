		...
		public override void Apply(Path p, ModifierData source)
		{
			...
			for (int i = 0; i < this.iterations; i++)
			{
				if (this.subdivideEveryIter && i != 0)
				{
					...
					{
						Vector3 vector = RaycastModifier.nodes[k];
						Vector3 vector2 = RaycastModifier.nodes[k + 1];
						RaycastModifier.nodes[k * 3] = RaycastModifier.nodes[k];
						if (k != count - 1)
						{
							RaycastModifier.nodes[k * 3 + 1] = Vector3.Lerp(vector, vector2, 0.33f);
							RaycastModifier.nodes[k * 3 + 2] = Vector3.Lerp(vector, vector2, 0.66f);
						}
					}
				}
				int l = 0;
		...
		public bool ValidateLine(GraphNode n1, GraphNode n2, Vector3 v1, Vector3 v2)
		{
			if (this.useRaycasting)
			{
				...
				if (this.thickRaycast && this.thickRaycastRadius > 0f)
				{
					...
					if (Physics.SphereCast(v1 + this.raycastOffset, this.thickRaycastRadius, v2 - v1, ref raycastHit, (v2 - v1).magnitude, this.mask))
					{
						return false;
					}
				}
				else if (Physics.Linecast(v1 + this.raycastOffset, v2 + this.raycastOffset, ref raycastHit2, this.mask))
				{
					return false;
				}
			}
