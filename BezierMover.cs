...
public class BezierMover : MonoBehaviour
{
	...
	private void Move(bool progress)
	{
		...
		while (num2 - num > 0.0001f)
		{
			...
			Vector3 vector = this.Plot(num3);
			if ((vector - base.transform.position).sqrMagnitude > this.speed * Time.deltaTime * (this.speed * Time.deltaTime))
			{
				num2 = num3;
			}
			else
	...
	public void OnDrawGizmos()
	{
		if (this.points.Length >= 3)
		{
			...
			for (int j = 0; j < this.points.Length; j++)
			{
				...
				Vector3 vector = this.points[j].position;
				for (int k = 1; k <= 100; k++)
				{
					Vector3 vector2 = AstarMath.CubicBezier(this.points[j].position, this.points[j].position + normalized * this.tangentLengths, this.points[(j + 1) % num].position - normalized2 * this.tangentLengths, this.points[(j + 1) % num].position, (float)k / 100f);
