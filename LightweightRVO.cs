...
public class LightweightRVO : MonoBehaviour
{
	...
	public void CreateAgents(int num)
	{
		...
		if (this.type == LightweightRVO.RVOExampleType.Circle)
		{
			...
			{
				Vector3 vector = new Vector3(Mathf.Cos((float)i * 3.14159274f * 2f / (float)this.agentCount), 0f, Mathf.Sin((float)i * 3.14159274f * 2f / (float)this.agentCount)) * num2;
		...
		else if (this.type == LightweightRVO.RVOExampleType.Line)
		{
			...
			{
				Vector3 position;
				position..ctor((float)((j % 2 != 0) ? -1 : 1) * this.exampleScale, 0f, (float)(j / 2) * this.radius * 2.5f);
		...
		{
			float num3 = Mathf.Sqrt((float)this.agentCount * this.radius * this.radius * 4f / 3.14159274f) * this.exampleScale * 0.05f;
			for (int l = 0; l < this.agentCount; l++)
			{
				float num4 = Random.value * 3.14159274f * 2f;
				float num5 = Random.value * 3.14159274f * 2f;
				Vector3 position3 = new Vector3(Mathf.Cos(num4), 0f, Mathf.Sin(num4)) * this.uniformDistance(num3);
				IAgent item4 = this.sim.AddAgent(position3);
				this.agents.Add(item4);
				this.goals.Add(new Vector3(Mathf.Cos(num5), 0f, Mathf.Sin(num5)) * this.uniformDistance(num3));
				this.colors.Add(LightweightRVO.HSVToRGB(num5 * 57.29578f, 0.8f, 0.6f));
			}
		}
	...
	public void Update()
	{
		...
		for (int k = 0; k < this.agents.Count; k++)
		{
			...
			{
				vector2..ctor(0f, 0f, agent.Radius);
			}
			Vector3 vector3 = Vector3.Cross(Vector3.up, vector2);
			Vector3 vector4 = agent.InterpolatedPosition + this.renderingOffset;
			int num = 4 * k;
			int num2 = 6 * k;
			this.verts[num] = vector4 + vector2 - vector3;
			this.verts[num + 1] = vector4 + vector2 + vector3;
			this.verts[num + 2] = vector4 - vector2 + vector3;
			this.verts[num + 3] = vector4 - vector2 - vector3;
